-- Corrective migration: fix DELETE statements in replace_venues_v3
-- to satisfy pg-safeupdate (requires WHERE clause on all DELETE statements).
--
-- Original migration: 20260809000001_create_replace_venues_v3_rpc.sql
-- Problem: DELETE FROM user_favorites; and DELETE FROM venues;
--          fail with "DELETE requires a WHERE clause" due to pg-safeupdate.
-- Fix: Add WHERE clauses that select all legitimate rows:
--   - DELETE FROM user_favorites WHERE user_id IS NOT NULL;
--   - DELETE FROM venues WHERE id IS NOT NULL;
...
BEGIN
  ...
  -- safeupdate fix: WHERE user_id IS NOT NULL (user_id is non-nullable)
  ...
  -- safeupdate fix: WHERE id IS NOT NULL (id is non-nullable UUID primary key)
