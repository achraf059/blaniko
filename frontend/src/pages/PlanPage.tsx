...
import {
  ...
  buildRefinedSelection,
  createRoleCategoryPreferences,
  mapQuizAnswersToStyle,
  scoreVenuesWithSeed as scoreVenuesWithSeedEngine,
  stopRoleOrder,
  type PlanStyleId,
  type StopRoleKey,
...
export default function PlanPage() {
  ...
  // Thin wrapper over the pure engine — keeps every existing call site
  // (scoreVenuesWithSeed(refreshSeed), scoreVenuesWithSeed(nextSeed)) unchanged.
  const scoreVenuesWithSeed = (seed: number) =>
    scoreVenuesWithSeedEngine(venues, seed, {
  ...
  // Delegates lock/replace/shuffle selection to the pure engine, then re-attaches
  // localized role/roleHint copy. Only venue identity is observable downstream (the
  // URL persists slugs), so copy is derived here rather than inside the engine.
  const buildRefinedStops = (options: {
    ...
    const selection = buildRefinedSelection({
      rankedVenues: options.rankedVenues,
      baseSelection: options.baseStops.map((stop, index) => ({
        roleKey: stopRoleOrder[Math.min(index, stopRoleOrder.length - 1)],
        venue: stop.venue,
      })),
      locked: options.locked,
      replaceRole: options.replaceRole,
      rolePreferences,
      style: selectedPlanStyle,
      mood: selectedMood,
      selectedCategory,
    ...
    return selection.map(({ roleKey, venue }) => ({
  ...
  // Initial START → MAIN → FINISH selection via the pure engine, then mapped to
  // rendered OutingStops with localized role/roleHint copy (unchanged behavior).
  const initialSelection = buildInitialSelection({
    scoredVenues,
    rolePreferences,
    style: selectedPlanStyle,
    mood: selectedMood,
  ...
  const planStops: OutingStop[] = initialSelection.map(({ roleKey, venue }) => ({
    role: stopRoleLabel[roleKey],
    roleHint: roleHintsForPlan[roleKey],
    venue,
  }));
